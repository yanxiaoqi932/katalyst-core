...
// key means node name
type allNodesNUMAAffinityCount map[string]util.TopologyAffinityCount
...
type NUMAInfoList []util.NumaInfo
...
func (nc allNodesNUMAAffinityCount) clone() allNodesNUMAAffinityCount {
	...
	for nodeUID, tc := range nc {
		newTc[nodeUID] = make(util.TopologyAffinityCount, len(tc))
...
func (na *NUMAInterPodAffinity) PreFilter(ctx context.Context, cycleState *framework.CycleState, pod *v1.Pod) (*framework.PreFilterResult, *framework.Status) {
	...
	podAffinity, err := util.UnmarshalAffinity(pod.Annotations)
	if err != nil {
	...
	podAffinityInfo := util.RequiredPodAffinityInfo(podAffinity, pod.Labels)
...
func (na *NUMAInterPodAffinity) getExistingAntiAffinityCounts(ctx context.Context, podAffinityInfo util.PodInfo,
	...
	for i := 0; i < len(nodesInfo); i++ {
		...
		go func(nodeIndex int) {
			...
			util.GetExistingAntiAffinityCounts(&state, allNodesSocket2NumaList[nodeName])
			lk.Lock()
...
func (na *NUMAInterPodAffinity) getAntiAffinityCounts(ctx context.Context, podAffinityInfo util.PodInfo,
	allNodesNUMAInfoList map[string]NUMAInfoList, nodesInfo []*framework.NodeInfo,
	allNodesSocket2NumaList map[string]socket2numaList) allNodesNUMAAffinityCount {
	topoMaps := make(allNodesNUMAAffinityCount)
	...
	for i := 0; i < len(nodesInfo); i++ {
		...
		go func(nodeIndex int) {
			...
			util.GetAntiAffinityCounts(&state, allNodesSocket2NumaList[nodeName])
			lk.Lock()
...
func (na *NUMAInterPodAffinity) getAffinityCounts(ctx context.Context, podAffinityInfo util.PodInfo,
	allNodesNUMAInfoList map[string]NUMAInfoList, nodesInfo []*framework.NodeInfo,
	allNodesSocket2NumaList map[string]socket2numaList) allNodesNUMAAffinityCount {
	topoMaps := make(allNodesNUMAAffinityCount)
	...
	for i := 0; i < len(nodesInfo); i++ {
		...
		go func(nodeIndex int) {
			...
			util.GetAffinityCounts(&state, allNodesSocket2NumaList[nodeName])
			lk.Lock()
	...
	for _, topologyZone := range zones {
		...
		for _, child := range topologyZone.Children {
			...
			for _, allocation := range child.Allocations {
				...
				labels = util.MergeNumaInfoMap(pod.Labels, labels)
				podAffinity, err := util.UnmarshalAffinity(pod.Annotations)
				if err != nil {
					klog.Error(err)
					continue
			...
			for _, podAffinity := range assumedPodAffinity {
				labels = util.MergeNumaInfoMap(podAffinity.Labels, labels)
				podAffinity, err := util.UnmarshalAffinity(podAffinity.Annotations)
				if err != nil {
					klog.Error(err)
					continue
				}
			...
			NUMAInfoList = append(NUMAInfoList, util.NumaInfo{
				Labels:                        labels,
				SocketID:                      socketID,
				NumaID:                        numaID,
